...
const CreateBundledProduct = () => {
  ...
  useQuery({
    queryKey: ["products", currnetProductPage],
    queryFn: async () => {
      ...
      // Filter out bundle wrapper products (they have a single "Default" option with "default" value)
      const nonBundleProducts = products.filter(product => {
        const hasDefaultOption = product.options?.some(opt => {
          const values = opt.values?.map(v => typeof v === 'string' ? v : v.value) || []
          return opt.title === "Default" && values.includes("default")
        })
        // If it has the Default option AND only one variant, it's likely a bundle wrapper
        const isBundleWrapper = hasDefaultOption && product.variants?.length === 1
        return !isBundleWrapper
      })
      setProducts((prev) => [...prev, ...nonBundleProducts])
