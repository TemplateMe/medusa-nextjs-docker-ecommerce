...
export const applyLoyaltyOnCartWorkflow = createWorkflow(
  ...
  (input: WorkflowInput) => {
    ...
    const loyaltyData = getCartLoyaltyPromoAmountStep({
      cart: carts[0]
    ...
    const { metadata, maxUsablePoints, ...updatePromoData } = transform({
      carts,
      promoToCreate,
      loyaltyPromo,
      loyaltyData
      ...
      return {
        ...
        metadata: {
          ...
          loyalty_points_used: data.loyaltyData.pointsUsed,
          loyalty_discount_amount: data.loyaltyData.amount
        },
        maxUsablePoints: data.loyaltyData.pointsUsed
      }
    })
